...
func (o *OrchestrationService) processBackgroundTile(coords entities.TileCoordinates) {
	...
	data, status, _ := o.cache.GetTileWithStatus(coords)
	...
	if status != interfaces.TileValid {
		...
		if err := o.cache.StoreTile(coords, data); err != nil {
			log.Printf("Failed to store background tile in cache: %v", err)
		}
	}

	// Always store to backup (even if tile was already valid in cache)
	if o.backup != nil {
		if err := o.backup.StoreTile(coords, data); err != nil {
			log.Printf("Failed to store tile in backup: %v", err)
		}
	}
}
