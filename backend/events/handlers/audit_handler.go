...
// AuditHandler handles events for audit logging
...
func (h *AuditHandler) HandleTrailEvent(ctx context.Context, event interfaces.Event) error {
...
func (h *AuditHandler) HandleEngagementEvent(ctx context.Context, event interfaces.Event) error {
...
func (h *AuditHandler) HandleUserEvent(ctx context.Context, event interfaces.Event) error {
...
// Helper function to calculate changes between trail versions
func calculateTrailChanges(previous, current *entities.Trail) map[string]interface{} {
	...
	if previous.Level != current.Level {
		changes["level"] = map[string]interface{}{
			"from": string(previous.Level),
			"to":   string(current.Level),
		}
	}
	...
	// Compare GPX file
	if previous.GPXFile != current.GPXFile {
		changes["gpx_file"] = map[string]interface{}{
			"from": previous.GPXFile,
			"to":   current.GPXFile,
		}
	}

	// Compare distance
	if (previous.DistanceM == nil) != (current.DistanceM == nil) ||
		(previous.DistanceM != nil && current.DistanceM != nil && *previous.DistanceM != *current.DistanceM) {
		changes["distance_m"] = map[string]interface{}{
			"from": previous.DistanceM,
			"to":   current.DistanceM,
		}
	}
