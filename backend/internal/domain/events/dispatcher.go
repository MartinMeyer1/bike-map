...
type Handler func(ctx context.Context, event event_types.Event) error
...
func (d *Dispatcher) Publish(ctx context.Context, event event_types.Event) error {
	d.mutex.RLock()
...
func (d *Dispatcher) PublishSync(ctx context.Context, event event_types.Event) error {
	d.mutex.RLock()
	handlers, exists := d.handlers[event.Type()]
	d.mutex.RUnlock()
