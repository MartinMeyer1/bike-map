...
-- ============================================================================
-- FUNCTION: Regenerate MVT tiles for a list of tile coordinates
-- ============================================================================

CREATE OR REPLACE FUNCTION regenerate_mvt_tiles(p_tiles trail_tiles[])
RETURNS VOID AS $$
DECLARE
    v_tile RECORD;
    v_coords INTEGER[];
BEGIN
    ...
    FOR v_tile IN 
        ...
        FROM unnest(p_tiles) AS t(trail_id, z, x, y)
    LOOP
        -- Generate and upsert tile
...
-- ============================================================================
-- FUNCTION: Regenerate MVT tiles by coordinates directly
-- ============================================================================

CREATE OR REPLACE FUNCTION regenerate_tiles_for_coordinates(
    p_coordinates INTEGER[][3]  -- Array of [z, x, y]
)
RETURNS VOID AS $$
DECLARE
    v_coord INTEGER[];
BEGIN
    FOREACH v_coord SLICE 1 IN ARRAY p_coordinates
    LOOP
        INSERT INTO mvt_tiles (z, x, y, data, generated_at)
        VALUES (v_coord[1], v_coord[2], v_coord[3], generate_mvt_tile(v_coord[1], v_coord[2], v_coord[3]), NOW())
        ON CONFLICT (z, x, y) DO UPDATE 
            SET data = EXCLUDED.data,
                generated_at = EXCLUDED.generated_at;
    END LOOP;
