...
export class PocketBaseService {
  ...
  static async getTrailRatingStats(
    ...
    try {
      ...
      const ratingAverageResponse = await pb
      ...
      if (ratingAverageResponse.items.length > 0) {
        const ratingAverage = ratingAverageResponse
          .items[0] as unknown as RatingAverage;
      ...
      if (userId) {
        try {
          const userRatingResponse = await pb
            .collection("trail_ratings")
            .getList(1, 1, {
              filter: `trail = "${trailId}" && user = "${userId}"`,
            });
          userRating =
            userRatingResponse.items.length > 0
              ? userRatingResponse.items[0].rating
              : undefined;
        } catch (error) {
