...
export async function shareTrail(
  ...
  const shareData: ShareData = {
    ...
    text: `Check out this ${trail.level} trail on BikeMap!`,
    url: shareUrl,
  };
  ...
  if (canShare()) {
    try {
      ...
      if (navigator.canShare && !navigator.canShare(shareData)) {
        console.warn(
          "⚠️ Cannot share this specific data, falling back to clipboard",
        );
      ...
      console.warn(
        "⚠️ Web Share API failed, falling back to clipboard:",
        error,
      );
    }
  }
